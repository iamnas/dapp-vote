            ...
            <div className="p-1 sm:p-2">
                {loading && !isRefreshing ? (
                    ...
                    <div className={`flex flex-col items-center justify-center p-6 ${darkMode ? 'text-red-300' : 'text-red-600'}`}>
                        ...
                        <button
                            onClick={handleRefresh}
                            className={`px-3 py-1.5 rounded-md text-sm flex items-center gap-1.5 ${darkMode
                    ...
                    <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                        {sortedCandidates.map((candidate, index) => {
                            ...
                            return (
                                <li
                                    key={candidate.id}
                                    className={`relative p-3 sm:p-4 transition-colors ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-300'} 
                                    ${isLeader ? (darkMode ? 'bg-blue-900/10' : 'bg-blue-50/50') : ''}`}>

                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-full ${isLeader
                                            ...
                                            <div>
                                                ...
                                                {isLeader && (
                                                    <span className={`ml-2 text-xs px-1.5 py-0.5 rounded-full ${darkMode ? 'bg-yellow-900/60 text-yellow-200' : 'bg-yellow-100 text-yellow-800'
                                                        }`}>
                                                        Leading
                                                    </span>
                                                )}
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className={`text-xs px-2 py-1 rounded-full ${candidate.voteCount > 0
